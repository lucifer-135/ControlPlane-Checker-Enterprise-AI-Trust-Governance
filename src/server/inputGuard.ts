...
const SSN_INPUT_REGEX = /\b(?!000|666|9\d{2})\d{3}[- ](?!00)\d{2}[- ](?!0000)\d{4}\b/;
...
export interface InputGuardResult {
  ...
  riskScore: number;
  detections: string[];
  details?: Array<{ category: string; severity: number; matched?: string }>;
}
...
export function scanInput(input: string, apiKey?: string): InputGuardResult {
  ...
  if (apiKey && !checkRateLimit(apiKey)) {
    return {
      ...
      details: [{ category: 'rate_limit', severity: 1.0 }],
    };
  }
  ...
  if (blocked) {
    if (detections.some((d) => d.includes('injection'))) {
      reason = `Prompt injection attempt detected: ${detections[0]}`;
